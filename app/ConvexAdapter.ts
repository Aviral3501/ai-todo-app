...
export const ConvexAdapter: Adapter = {
  async createAuthenticator(authenticator: Authenticator) {
    await callMutation(api.autAdapter.createAuthenticator, {
      authenticator,
    } as any);
    return authenticator;
  },
  async createSession(session: Session) {
    const id = await callMutation(api.autAdapter.createSession, {
      session: toDB(session),
    });
    return { ...session, id };
  },
  async createUser({ id: _, ...user }: User) {
    const id = await callMutation(api.autAdapter.createUser, {
      user: toDB(user),
    });
    return { ...user, id };
  },
  async createVerificationToken(verificationToken: VerificationToken) {
    await callMutation(api.autAdapter.createVerificationToken, {
  ...
  async deleteSession(sessionToken) {
    return maybeSessionFromDB(
      await callMutation(api.autAdapter.deleteSession, {
        sessionToken,
      })
    );
  },
  async deleteUser(id: Id<"users">) {
    return maybeUserFromDB(
      await callMutation(api.autAdapter.deleteUser, { id })
    );
  },
  async getAccount(providerAccountId, provider) {
    return await callQuery(api.autAdapter.getAccount, {
      provider,
      providerAccountId,
    });
  },
  async getAuthenticator(credentialID) {
    return await callQuery(api.autAdapter.getAuthenticator, { credentialID });
  },
  async getSessionAndUser(sessionToken) {
    const result = await callQuery(api.autAdapter.getSessionAndUser, {
      sessionToken,
    });
    if (result === null) {
      return null;
  ...
  async getUser(id: Id<"users">) {
    return maybeUserFromDB(await callQuery(api.autAdapter.getUser, { id }));
  },
  async getUserByAccount({ provider, providerAccountId }) {
    return maybeUserFromDB(
      await callQuery(api.autAdapter.getUserByAccount, {
        provider,
        providerAccountId,
      })
    );
  ...
  async getUserByEmail(email) {
    return maybeUserFromDB(
      await callQuery(api.autAdapter.getUserByEmail, { email })
    );
  },
  async linkAccount(account: Account) {
    return await callMutation(api.autAdapter.linkAccount, { account });
  },
  async listAuthenticatorsByUserId(userId: Id<"users">) {
    return await callQuery(api.autAdapter.listAuthenticatorsByUserId, {
      userId,
    });
  },
  async unlinkAccount({ provider, providerAccountId }) {
    return (
      (await callMutation(api.autAdapter.unlinkAccount, {
        provider,
        providerAccountId,
      })) ?? undefined
    );
  ...
  async updateAuthenticatorCounter(credentialID, newCounter) {
    return await callMutation(api.autAdapter.updateAuthenticatorCounter, {
      credentialID,
      newCounter,
    });
  },
  async updateSession(session: Session) {
    return await callMutation(api.autAdapter.updateSession, {
      session: toDB(session),
    });
  },
  async updateUser(user: User) {
    await callMutation(api.autAdapter.updateUser, { user: toDB(user) });
    return user;
  },
  async useVerificationToken({ identifier, token }) {
    return maybeVerificationTokenFromDB(
      await callMutation(api.autAdapter.useVerificationToken, {
        identifier,
        token,
      })
    );
